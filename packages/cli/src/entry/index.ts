...
function createAppEntry() {
  ...
  function travelComponents(entry, page, pkg) {
    ...
    if (compMap) {
      Object.entries(compMap).forEach(([key, value]) => addComponent(key, value as string, page, pkg, parse(entry).dir))
    }
  }

  function addComponent(key = '', value = '', page = '', pkg = MAIN_PACKAGE, currentDir = '') {
